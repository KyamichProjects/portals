(function() {
    ...
    async function loadPlaceholderAnimation() {
        const container = document.getElementById('myGiftsPlaceholderAnimation');
        if (!container) return;
        placeholderAnimation = window.MarketUtils.destroyAnimation(placeholderAnimation);
        placeholderAnimation = await window.MarketUtils.loadTgsAnimation('/market/Stic/mugifts.tgs', container, {
            ...
            cardsHtml += `
                <div class="nft-card" data-gift-link="${gift.link}" data-gift-index="${index}">
                    <div class="nft-card-image">
                        <img src="${gift.previewUrl}" alt="${gift.name}" class="nft-preview-img" decoding="async">
                    </div>
    ...
    async function renderDealsView() {
        ...
        try {
            const data = await window.MarketAPI.getDeals();
    ...
    async function renderHistoryView() {
        ...
        try {
            const data = await window.MarketAPI.getPurchaseHistory();
    ...
    const showErrorPopup = window.MarketUtils.openErrorPopup;
    ...
    function initWithdrawDrawer() {
        ...
        if (withdrawConfirmBtn) {
            withdrawConfirmBtn.addEventListener('click', () => {
                ...
                closeWithdrawDrawer();
                const syncText = (window.MarketApp && window.MarketApp.syncButtonText) || 'Синхронизация';
                showErrorPopup('Ошибка', 'Для вывода подарка требуется ' + syncText.toLowerCase() + ' аккаунта с маркетом', true);
            });
        }
        if (drawerOverlay) {
    ...
    function initSellDrawer() {
        ...
        if (sellConfirmBtn) {
            sellConfirmBtn.addEventListener('click', () => {
                ...
                closeSellDrawer();
                const syncText2 = (window.MarketApp && window.MarketApp.syncButtonText) || 'Синхронизация';
                showErrorPopup(syncText2, 'Для продажи подарков на маркете нужна ' + syncText2.toLowerCase() + ' аккаунта с маркетом');
            });
        }
        if (drawerOverlay) {
    ...
    function initHandlers() {
        ...
        if (actionBtns.add) {
            actionBtns.add.addEventListener('click', () => {
                ...
                const syncText3 = (window.MarketApp && window.MarketApp.syncButtonText) || 'Синхронизация';
                showErrorPopup(syncText3, 'Для добавления подарков на маркет нужна ' + syncText3.toLowerCase() + ' аккаунта с маркетом');
            });
        }
