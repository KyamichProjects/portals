(function () {
    ...
    let cartNotificationInitialized = false;
        ...
        if (payBtn) {
            payBtn.addEventListener('click', async () => {
                ...
                for (const item of cart) {
                    try {
                        const data = await window.MarketAPI.buyGift(item.link, item.price);
    ...
    function initCartNotification() {
        if (cartNotificationInitialized) return;
        ...
        if (notification) {
            ...
            cartNotificationInitialized = true;
        }
    }
    ...
    function resetCartModule() {
        cartNotificationInitialized = false;
        cartDrawerInitialized = false;
    }
